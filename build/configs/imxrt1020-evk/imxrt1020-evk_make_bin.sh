...
MAKE_FLASHFILE() {
	if [[ "${CONFIG_APP_BINARY_SEPARATION}" == "y" ]]; then
		...
		for i in "${!NAMES[@]}"; do
			...
			elif [ "${NAMES[$i]}" = "userfs" ]; then
				let FS_PART=${SIZES[$i]}*1024
				echo "FS_PART="$FS_PART
			fi
		done
		...
		if [ ${OS_PART} -lt ${KERN_SIZE} -o ${USER_PART} -lt ${USER_SIZE} -o ${FS_PART} -lt ${FS_SIZE} ]; then
